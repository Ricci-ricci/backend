...
function getRandomFeatures(count = 3) {
...
async function main() {
    ...
    for (const file of imageFiles) {
        try {
            ...
            const features = getRandomFeatures(Math.floor(Math.random() * 3) + 2); // 2 to 4 features
            ...
            if (createdCategories.length === 0) {
                console.warn(`⚠️ No categories available; skipping product creation for image ${file}`);
                continue;
            }
            ...
            if (!randomCategory) {
                console.warn(`⚠️ Failed to pick a random category; skipping product creation for image ${file}`);
                continue;
            }
            ...
            console.error(`❌ Failed to create product for image ${file}:`, error);
        }
    }
